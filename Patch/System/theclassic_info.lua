function main()
	for ItemID, DESC in pairs(tbl) do
		...
		if (DisplayServer == 3 and DESC.Server ~= nil) or (DisplayCustomServer == 3 and DESC.Custom == true) or DisplayItemID == 2 or DisplayDatabase == true or DisplayCustomDB == true then
			AddItemIdentifiedDesc(ItemID, "--------------------------")
			AddItemIdentifiedDesc(ItemID, "CashPOINTS: <URL>[Adquirir]<INFO>https://theclassic.games/donate</INFO></URL>")
			AddItemIdentifiedDesc(ItemID, "Discord: <URL>[Participar]<INFO>https://discord.gg/theclassicgames</INFO></URL>")
			...
			if DisplayItemID == 2 then
				AddItemIdentifiedDesc(ItemID, "^0000CC[Database ID]:^000000 "..ItemID)
			end
			end
		end
