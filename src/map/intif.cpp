 ...
 * @return 
 */
int32 intif_create_pet(uint32 account_id,uint32 char_id,int16 pet_class,int16 pet_lv, t_itemid pet_egg_id, t_itemid pet_equip,int16 intimate,int16 hungry,char rename_flag,char incubate,const char *pet_name, int16 str, int16 agi, int16 vit, int16 int_, int16 dex, int16 luk)
{
	if (CheckForCharServer())
		return 0;
	WFIFOHEAD(inter_fd, 40 + NAME_LENGTH);
	...
	WFIFOW(inter_fd, 28) = str;
	WFIFOW(inter_fd, 30) = agi;
	WFIFOW(inter_fd, 32) = vit;
	WFIFOW(inter_fd, 34) = int_;
	WFIFOW(inter_fd, 36) = dex;
	WFIFOW(inter_fd, 38) = luk;
	safestrncpy(WFIFOCP(inter_fd, 40), pet_name, NAME_LENGTH);
	WFIFOSET(inter_fd, 40 + NAME_LENGTH);
