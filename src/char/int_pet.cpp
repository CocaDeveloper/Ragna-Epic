...
{
	//`pet` (`pet_id`, `class`,`name`,`account_id`,`char_id`,`level`,`str`,`agi`,`vit`,`int`,`dex`,`luk`,`egg_id`,`equip`,`intimate`,`hungry`,`rename_flag`,`incubate`,`autofeed`)
	...
	{// New pet.
		if( SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` "
			"(`class`,`name`,`account_id`,`char_id`,`level`,`str`,`agi`,`vit`,`int`,`dex`,`luk`,`egg_id`,`equip`,`intimate`,`hungry`,`rename_flag`,`incubate`,`autofeed`) "
			"VALUES ('%d', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', '%u', '%d', '%d', '%d', '%d', '%d')",
	...
	{// Update pet.
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `class`='%d',`name`='%s',`account_id`='%d',`char_id`='%d',`level`='%d',`str`='%d',`agi`='%d',`vit`='%d',`int`='%d',`dex`='%d',`luk`='%d',`egg_id`='%u',`equip`='%u',`intimate`='%d',`hungry`='%d',`rename_flag`='%d',`incubate`='%d',`autofeed`='%d' WHERE `pet_id`='%d'",
			schema_config.pet_db, p->class_, esc_name, p->account_id, p->char_id, p->level,
...
int32 inter_pet_fromsql(int32 pet_id, struct s_pet* p)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `pet_id`, `class`,`name`,`account_id`,`char_id`,`level`,`str`,`agi`,`vit`,`int`,`dex`,`luk`,`egg_id`,`equip`,`intimate`,`hungry`,`rename_flag`,`incubate`,`autofeed` FROM `%s` WHERE `pet_id`='%d'", schema_config.pet_db, pet_id) )
	{
		Sql_ShowDebug(sql_handle);
		return 0;
	}
	...
	if( SQL_SUCCESS == Sql_NextRow(sql_handle) )
	{
		...
		Sql_GetData(sql_handle,  6, &data, nullptr); p->str = atoi(data);
		Sql_GetData(sql_handle,  7, &data, nullptr); p->agi = atoi(data);
		Sql_GetData(sql_handle,  8, &data, nullptr); p->vit = atoi(data);
		Sql_GetData(sql_handle,  9, &data, nullptr); p->int_ = atoi(data);
		Sql_GetData(sql_handle, 10, &data, nullptr); p->dex = atoi(data);
		Sql_GetData(sql_handle, 11, &data, nullptr); p->luk = atoi(data);
		Sql_GetData(sql_handle, 12, &data, nullptr); p->egg_id = strtoul(data, nullptr, 10);
		Sql_GetData(sql_handle, 13, &data, nullptr); p->equip = strtoul(data, nullptr, 10);
		Sql_GetData(sql_handle, 14, &data, nullptr); p->intimate = atoi(data);
		Sql_GetData(sql_handle, 15, &data, nullptr); p->hungry = atoi(data);
		Sql_GetData(sql_handle, 16, &data, nullptr); p->rename_flag = atoi(data);
		Sql_GetData(sql_handle, 17, &data, nullptr); p->incubate = atoi(data);
		Sql_GetData(sql_handle, 18, &data, nullptr); p->autofeed = atoi(data) != 0;
...
int32 mapif_create_pet(int32 fd, uint32 account_id, uint32 char_id, int16 pet_class, int16 pet_lv, t_itemid pet_egg_id, t_itemid pet_equip, int16 intimate, int16 hungry, char rename_flag, char incubate, char *pet_name, int32 str, int32 agi, int32 vit, int32 int_, int32 dex, int32 luk)
{
...
int32 mapif_parse_CreatePet(int32 fd){
	const char *name = RFIFOCP(fd, 40);
	mapif_create_pet(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOW(fd, 10), RFIFOW(fd, 12), RFIFOL(fd, 14), RFIFOL(fd, 18), RFIFOW(fd, 22),
		RFIFOW(fd, 24), RFIFOB(fd, 26), RFIFOB(fd, 27), const_cast<char*>(name), RFIFOW(fd, 28), RFIFOW(fd, 30), RFIFOW(fd, 32),
		RFIFOW(fd, 34), RFIFOW(fd, 36), RFIFOW(fd, 38));
	return 0;
}
